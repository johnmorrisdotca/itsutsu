 ...
 * /games/<slug>/<id>/<move> from the local move index as soon as the match
...
test.describe("refreshing straight after a move", () => {
  test("finds the game, rather than saying there is none", async ({ page }) => {
     ...
     */
    await page.waitForURL(/\/games\/gomoku\/[a-z0-9-]+\/\d+$/, { timeout: 15_000 });
  ...
  test("holds the address back while the record catches up, then follows it", async ({ page }) => {
     ...
     * purpose. Without the fix the bar says /games/gomoku/<id>/2 while the
     ...
     * played two, with nothing to tell them so. Naming the game rather than a
     * position in it is honest, and a refresh restores what they had.
     */
    expect(held, "the bar named a position rather than the game").toMatch(/\/games\/gomoku$/);

    // And once the record catches up, the bar follows rather than sticking.
    await page.unroute("**/api/games/*/moves");
    await page.waitForURL(/\/games\/gomoku\/[a-z0-9-]+\/2$/, { timeout: 20_000 });
  ...
  test("keeps naming a position the record actually holds", async ({ page }) => {
    ...
    for (const [row, col] of [[7, 7], [7, 8], [8, 7], [8, 8], [9, 7], [9, 8]] as const) {
      ...
      const url = page.url();
      if (!/\/games\/gomoku\/[a-z0-9-]+\/\d+$/.test(url)) continue;
