...
function summary(
  ...
  lines.push("## The door");
  lines.push("");
  lines.push("`PATCH /api/backlog/:id` with `releasedIn` (and `releasedAt`, if the instant is");
  lines.push("known) and no `status` is a release stamp. `stampRelease` in `backlogStore.ts`");
  lines.push("writes the two release columns onto a row that is already `done` and unstamped,");
  lines.push("and nothing else — no status, no `movedAt`, no claim — so BOARD_RULES.md");
  lines.push("invariant 1 (\"a done row does not move\") and 9 (\"the release tool writes");
  lines.push("`done`\") both still hold: it grants no new way to REACH done. It refuses, as a");
  lines.push("4xx with its reason, a row that is not done, a row already stamped, and a");
  lines.push("version `CHANGELOG.md` does not name; the rules are `stampProblems` in");
  lines.push("`backlog.ts`, unit-tested beside it. Open to the operator's session and to the");
  lines.push("board token; `pnpm task stamp <key> --release 0.x.y [--at <ISO>] --by \"<who>\"`");
  lines.push("is the same door from a terminal.");
  lines.push("");
  lines.push("`pnpm board:released-in` is a report by default and writes the `certain` rows");
  lines.push("with `--run` — and only with `BOARD_URL` named in the environment, so a write");
  lines.push("is asked for twice and never assumes the live site was meant. Before the door");
  lines.push("existed, the same PATCH answered 200 and stamped nothing, which is why the");
  lines.push("script re-reads every row it writes and stops on the first that did not take.");
  lines.push("");
