/**
 * The wordlist a four-word phrase is drawn from: EFF's SHORT list, verbatim.
 *
 * https://www.eff.org/files/2016/09/08/eff_short_wordlist_1.txt — 1,296 words,
 * every one three to five letters, already stripped by EFF of offensive and
 * confusable entries. Their reasoning is at https://www.eff.org/dice. The file
 * pairs each word with a five-digit dice roll; the rolls are for people
 * rolling actual dice and are discarded here.
 *
 * VENDORED, never fetched at runtime. A credential that cannot be minted
 * because eff.org is slow is a credential that does not work, and a list that
 * could change under us is a list that could invalidate somebody's phrase.
 *
 * THE SHORT LIST AND NOT THE LONG ONE, which is a decision with a reason
 * rather than the first file that came to hand. The long list is 7,776 words
 * and about 51 bits for four of them; this one is 1,296 and about 41. Those ten
 * bits are given up deliberately. The long list holds "abdominal" and "abacus",
 * and the person this feature exists for is a twelve-year-old who has to keep
 * four of these in her head — a phrase nobody can remember is a credential
 * nobody can use, whatever it scores. Forty-one bits against a rate-limited
 * entry path is already far past what is needed.
 *
 * DO NOT ADD A FIFTH WORD, and do not add numerals, to win those bits back. A
 * fifth word is a fifth thing to remember; numerals do not attach to images the
 * way words do, so they are the part a person forgets, and they want a keypad
 * where a word wants one tap. The backlog ticket records the same conclusion.
 *
 * NOT EFF's second short list (eff_short_wordlist_2_0.txt). That one is built
 * around unique three-letter prefixes, which solves a TYPING problem. Nothing
 * about this phrase is typed.
 *
 * One entry carries a hyphen — "yo-yo" — which is why a phrase is joined with a
 * space and never a hyphen. See `PHRASE_SEPARATOR` in phrase.ts.
 */
