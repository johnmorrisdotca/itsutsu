...
/** A Gomoku win on 15×15 on the board, even, unrated, the first of the day: 100 and 20. */
...
describe("what each result pays, as a share of the most", () => {
  it("pays a win 100% and a loss played out 20%", () => {
    expect(gamePoints(plain)).toEqual({ black: 100, white: 20 });
    expect(gamePoints({ ...plain, winner: "white" })).toEqual({ black: 20, white: 100 });
  });
  ...
  it("pays a close loss more: Reversi lost 30 discs to 34", () => {
    const reversi = { ...plain, variant: "reversi" as const, size: 8, reason: "count", score: { black: 34, white: 30 } };
    expect(gamePoints(reversi)).toEqual({ black: 100, white: 38 });
  });

  it("pays resigning, and the clock, less, and a resignation before the tenth move hardly at all", () => {
    expect(gamePoints({ ...plain, reason: "resign" })).toEqual({ black: 100, white: 10 });
    expect(gamePoints({ ...plain, reason: "resign", moveCount: 4 })).toEqual({ black: 50, white: 0 });
    expect(gamePoints({ ...plain, reason: "time" })).toEqual({ black: 80, white: 10 });
  });
  ...
  it("pays the same two players less the more they play each other in a day", () => {
    expect(gamePoints({ ...plain, earlierToday: 1 })).toEqual({ black: 50, white: 10 });
    expect(gamePoints({ ...plain, earlierToday: 2 })).toEqual({ black: 25, white: 5 });
    expect(gamePoints({ ...plain, earlierToday: 9 })).toEqual({ black: 25, white: 5 });
  });
