...
import type { BoardChange, BoardDraft, BoardMoveTarget, BoardOutcome, BoardTicketView } from "./boardClient.types.ts";
import type { SumilabuTarget } from "./sumilabuProject.types.ts";

/**
 * Itsutsu's features board, which lives on Sumilabu.
 *
 * One thin client for every caller — `pnpm task`, `pnpm release:take --done`,
 * and the /backlog page through `backlogStore.ts` — in UmaKuma's pattern. The
...
async function call(
  ...
  path: string,
  init: { method?: string; body?: unknown; actor?: string | null } = {},
...
/** Everything, or everything not yet done or dropped (stale holds included). */
export async function listTickets(target: SumilabuTarget, filter: { unfinished?: boolean } = {}): Promise<BacklogItem[]> {
  ...
  return (body.tickets as BoardTicketView[]).map(itemFromTicket);
}
