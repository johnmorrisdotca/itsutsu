 ...
 * once.
 *
 * Fixed rather than configurable: this is a backstop against a board nobody
 * could actually be keeping up with, not a setting anybody tunes. Twenty
 * boards is already more than a person plays in a week on a site where a
 * game can take days between moves — a twenty-first is not the one that was
 * waiting on them.
 *
...
export async function memberOverActiveLimit(
  ...
  for (const id of candidates) {
    if ((await activeGameCount(id)) >= ACTIVE_GAME_LIMIT) return id;
  }
  return null;
}
