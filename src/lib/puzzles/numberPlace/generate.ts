...
function fillGrid(size: number, random: Random): Grid {
  ...
  const fill = (index: number): boolean => {
    ...
    for (const value of shuffled(values, random)) {
      const bit = 1 << value;
      if ((rows[row] | cols[col] | boxes[box]) & bit) continue;
      grid[index] = value;
      rows[row] |= bit;
      cols[col] |= bit;
      boxes[box] |= bit;
...
export function generateNumberPlace(size: number, level: PuzzleLevel, seed: number): Puzzle {
  ...
  const solution = fillGrid(size, random);
  const { depth, floor } = LEVELS[level];
  const givens = [...solution];
  let left = givens.length;
  for (const index of shuffled(givens.map((_, i) => i), random)) {
    if (left <= floor[size]) break;
    const value = givens[index];
    givens[index] = 0;
    const stillOne = countSolutions(givens, size, 2) === 1 && guessDepth(givens, size) <= depth;
    if (stillOne) left -= 1;
