...
describe("what a kana word scores", () => {
  ...
  it("pays for the word, the rows left and the speed only when found", () => {
    ...
    expect(found.found).toBe(250 + 25 * 4);
  ...
  it("is always more for a word found than for any word lost", () => {
    for (const word of ["さくら", "べんとう", "えいきょう"]) {
      ...
      const bestLoss = kanaScore(word, [nearly, last, ...new Array<string>(KANA_ROWS - 2).fill(filler)], KANA_ROWS, 0).total;
      const worstWin = kanaScore(word, [...new Array<string>(KANA_ROWS - 1).fill(filler), word], KANA_ROWS, 60 * 60_000).total;
