...
describe("drawing a Gomoji Mot (French)", () => {
  ...
  it("checks a French word the way English's is checked, through the same server function", () => {
    ...
    expect(checkSolution("gomojiMot", 5, puzzle.givens, puzzle.solution)).toEqual({ ok: true });
    expect(checkSolution("gomojiMot", 5, puzzle.givens, `zzzzz${puzzle.solution}`).ok).toBe(false);
    // An English guess with no French meaning is refused as an unknown word, not accepted.
    expect(checkSolution("gomojiMot", 5, puzzle.givens, "zzzzz").ok).toBe(false);
  });
});
...
describe("drawing a Gomoji Wort (German)", () => {
  ...
  it("decodes and checks a German word carrying an umlaut", () => {
    ...
    expect(checkOutOfGuesses("gomojiWort", size, givens, umlautWord.repeat(size + 1)).ok).toBe(false);
  });
