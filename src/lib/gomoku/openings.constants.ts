...
export const RULES_ATTRIBUTION = [
  ...
  `The puzzles are named the same way. ${PUZZLE_DISPLAY.numberPlace.label} is our version of the puzzle sold as Sudoku, under the name it was first printed with; Sudoku is a trademark of Nikoli in Japan. ${PUZZLE_DISPLAY.jigsaw.label}, ${PUZZLE_DISPLAY.diagonal.label} and ${PUZZLE_DISPLAY.sumCages.label} are our versions of its commonest variants, sold as Jigsaw Sudoku, Sudoku X and Killer Sudoku. ${PUZZLE_DISPLAY.hiddenStones.label} is our version of the one-star form of Star Battle, which is also played daily as Queens, a name that belongs to LinkedIn; ${PUZZLE_DISPLAY.moreOrLess.label} is our version of Futoshiki, and ${PUZZLE_DISPLAY.towers.label} of the puzzle known as Skyscrapers. ${PUZZLE_DISPLAY.blackAndWhite.label} is our version of the binary puzzle sold as Takuzu and Binairo, names that are trademarks in the EU, which LinkedIn plays in a form of its own as Tango, a name that belongs to LinkedIn. Each is made by our own code from its rules, and the names appear here only to say what a puzzle resembles.`,
] as const;
