...
export const RULES_ATTRIBUTION = [
  ...
  // The puzzles' names are read from their table, so a rename is one edit there.
  `The puzzles are ours too, made by our own code from their rules, though most go by the names you already know. ${PUZZLE_DISPLAY.numberPlace.label}, ${PUZZLE_DISPLAY.jigsaw.label}, ${PUZZLE_DISPLAY.diagonal.label} and ${PUZZLE_DISPLAY.sumCages.label} are the puzzle Howard Garns first printed as Number Place and its commonest variants; Sudoku 数独 is a trademark of Nikoli in Japan, so in Japanese we call them ナンプレ, as Japanese publishers do. ${PUZZLE_DISPLAY.moreOrLess.label} is Tamaki Seimiya's puzzle, which Nikoli published, and ${PUZZLE_DISPLAY.towers.label} the grid puzzle of that name. ${PUZZLE_DISPLAY.hiddenStones.label} is our version of the one-star form of Star Battle, which is also played daily as Queens, a name that belongs to LinkedIn; ${PUZZLE_DISPLAY.blackAndWhite.label} is our version of the binary puzzle sold as Takuzu and Binairo, names that are trademarks in the EU, which LinkedIn plays in a form of its own as Tango, a name that belongs to LinkedIn. ${PUZZLE_DISPLAY.wordDrop.label} is our version of the word-guessing game published as Wordle, a trademark of The New York Times Company; its words are our own list, made from SCOWL (Kevin Atkinson). None of these owners has any connection with this site.`,
] as const;
