...
export const RULES_ATTRIBUTION = [
  ...
  `The puzzles are named the same way. ${PUZZLE_DISPLAY.numberPlace.label} is our version of the puzzle sold as Sudoku, under the name it was first printed with; Sudoku is a trademark of Nikoli in Japan. ${PUZZLE_DISPLAY.jigsaw.label} and ${PUZZLE_DISPLAY.diagonal.label} are our versions of its two commonest variants, sold as Jigsaw Sudoku and Sudoku X. ${PUZZLE_DISPLAY.hiddenStones.label} is our version of the one-star form of Star Battle, which is also played daily as Queens, a name that belongs to LinkedIn; ${PUZZLE_DISPLAY.moreOrLess.label} is our version of Futoshiki. Each is made by our own code from its rules, and the names appear here only to say what a puzzle resembles.`,
] as const;
