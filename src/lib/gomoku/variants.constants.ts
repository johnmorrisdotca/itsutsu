...
export const RULE_VARIANT_DISPLAY: Record<RuleVariant, VariantCopy> = {
  ...
  obstacleFive: {
    ...
    rules: [
      ...
      "Because a hotspot serves both sides, the same one can sit in a line of each colour. A stone only ever counts for its own colour, so one played in the gap of the other colour's line blocks it.",
  ...
  hotDrop: {
    ...
    rules: [
      "Stones fall to the bottom of their column.",
      "One random square is a hotspot: it is a stone of whichever colour is counting, so it can be part of your line or your opponent's.",
      "One random square is a hole that nothing can land on.",
      "Four in a row wins, the hotspot counting. A stone only ever counts for its own colour, so one dropped into the gap of your opponent's line blocks it.",
    ],
    board: "7×7 or 9×9.",
  },
  clearDrop: {
