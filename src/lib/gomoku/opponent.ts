...
import { DECIDED_SCORE, EVAL_WEIGHTS, REPLY_CAP, SEARCH, TIER_SPECS } from "./opponent.constants";
...
export function chooseTurn(
  ...
  if (spec.searchDepth > 0) {
    ...
    const searched =
      searchTurn(state, spec.searchDepth, random, left) ??
      lookAheadTurn(
        state,
     ...
     */
    const safe =
      entry !== undefined && (!guarding || readGuard(entry, me, until) === GUARD.cleared);
