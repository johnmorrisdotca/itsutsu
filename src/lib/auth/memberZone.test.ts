 ...
 *
 * The row is the signed-in member's, however they came in — by Google or with an
 * invite code — so the fake stands in for `currentMemberRow`.
 */

const currentMemberRow =
  vi.fn<() => Promise<{ timeZone: string | null; country: string | null; preferences?: unknown } | null>>();

vi.mock("./currentSession", () => ({ currentMemberRow }));

const { dayZoneUnknown } = await import("./memberZone");

beforeEach(() => {
  currentMemberRow.mockReset();
});
...
describe("do we know when this member's day ends", () => {
  it("yes, we do not: an empty column is every member who never opened their profile", async () => {
    currentMemberRow.mockResolvedValue({ timeZone: "", country: "" });
    expect(await dayZoneUnknown()).toBe(true);
  });

  it("no: a zone they chose is theirs and must never be written over", async () => {
    currentMemberRow.mockResolvedValue({ timeZone: "America/Vancouver", country: "Canada" });
    expect(await dayZoneUnknown()).toBe(false);
  });
  ...
  it("no: UTC they chose themselves is a choice, not the floor", async () => {
     ...
     */
    currentMemberRow.mockResolvedValue({ timeZone: "UTC", country: "" });
    expect(await dayZoneUnknown()).toBe(false);
  });
  ...
  it("yes: a zone that is only the country's guess may still be improved on", async () => {
    ...
    currentMemberRow.mockResolvedValue({ timeZone: "America/Toronto", country: "Canada" });
    expect(await dayZoneUnknown()).toBe(true);
  });
  ...
  it("no: the same zone with no country behind it was nobody's guess", async () => {
    ...
    currentMemberRow.mockResolvedValue({ timeZone: "America/Toronto", country: "" });
    expect(await dayZoneUnknown()).toBe(false);
  });

  it("yes: a zone this platform cannot read is no better than none", async () => {
    currentMemberRow.mockResolvedValue({ timeZone: "Mars/Olympus", country: "" });
    expect(await dayZoneUnknown()).toBe(true);
  });
  ...
  it("no: a zone they CHOSE is theirs even when it is exactly the country's guess", async () => {
    ...
    currentMemberRow.mockResolvedValue({
  ...
  it("no: a zone their browser reported is not asked for again", async () => {
    currentMemberRow.mockResolvedValue({ timeZone: "America/Toronto", country: "Canada", preferences: { timeZoneFrom: "device" } });
    expect(await dayZoneUnknown()).toBe(false);
  });

  it("yes: a guess the row records as a guess may still be improved on", async () => {
    currentMemberRow.mockResolvedValue({ timeZone: "America/Toronto", country: "Canada", preferences: { timeZoneFrom: "country" } });
    expect(await dayZoneUnknown()).toBe(true);
  });

  it("asks nothing of a reader with no member: not signed in, or the operator with no row to record one on", async () => {
    currentMemberRow.mockResolvedValue(null);
    expect(await dayZoneUnknown()).toBe(false);
  });
});
