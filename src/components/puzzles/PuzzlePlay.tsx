...
export function PuzzlePlay({
  ...
  useEffect(() => {
    ...
    router.replace(`${playPath(kind)}${puzzleQuery({ size, level, seed: backwards ? freshBackwardsSeed() : freshSeed(), checks, hints, strict, headStart, backwards })}`);
  }, [seed, kind, size, level, checks, hints, strict, headStart, backwards, router]);
